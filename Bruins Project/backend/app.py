...
def roster(team_id):
    ...
    res = get_roster(team_id, params=request.args)
    return jsonify(res)

@app.route("/players/<player_id>/allStats")
def playerallStats(player_id):
    
    params = dict(stats='yearByYear')
    params.update(request.args)
    res = get_player_stats(player_id, params=params)
    return jsonify(res)

@app.route("/players/<player_id>/gameLog")
def playerGameLog(player_id):
    
    params = dict(stats='gameLog',season='20212022')
    params.update(request.args)
    res = get_player_stats(player_id, params=params)
    return jsonify(res)